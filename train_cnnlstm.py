...
tf.flags.DEFINE_integer("num_filters", 100, "Number of feature maps per each window size")
tf.flags.DEFINE_string("filter_sizes", "3,4,5", "Comma-separated filter sizes")
tf.flags.DEFINE_integer("max_pool_size", 4, "max pool window size")
tf.flags.DEFINE_string("lstm_type", "gru", "either 'gru', 'basic', or 'peephole'")
...
with tf.Graph().as_default():
    ...
    with sess.as_default():
        ...
        run_folder = 'cnnlstm_run' + int(FLAGS.vn)*'_vn'
        ...
        def dev_step(x_batch, y_batch, writer=None):
            ...
            feed_dict = {
              ...
              lstm.pad: np.zeros([len(x_batch), 1, FLAGS.embedding_dim, 1]),
              lstm.real_len: real_len(x_batch),
            }
